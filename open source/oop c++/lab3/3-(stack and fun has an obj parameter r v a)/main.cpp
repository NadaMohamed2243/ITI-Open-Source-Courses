...
int main()
{
    ...
    //by ref --> no constructor --> no destructor (only alias)
    myfun(s1);

    cout<<endl<<"-------------after calling function (by ref)(any change in s --> change s1)------------------------"<<endl;
    ...
    //by add --> no constructor --> no destructor (only pointer)
    myfun2(&s1);

    cout<<endl<<"-------------after calling function (by add)(any change in pointer s --> change s1)------------------------"<<endl;
    ...
    cout<<endl<<"-------------after calling function (by value)(any change in s --> change s1)------------------------"<<endl;


    // there is no array to acess
    if (s1.pop(x)==1)
    {
        cout<<endl<<x<<endl;  //rubbish data
    }
...
void myfun3(Stack s)
{
    ...
    //copy contain of s1 to s --> include the address in arr
    // so s and s1 point to the same place(array);
    //any change in s change array (don't chant tos of s1 )
    //at the end of myfun3 --> destractor destroy s (destroy array)
    // s1 array is deleted
    // s -->  {10 20 40 (60 70)};
